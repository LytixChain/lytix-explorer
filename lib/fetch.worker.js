...
const getCoins = async (query) => {
  try {
    ...
    const avgMAXTime = await fetch(`${ api }/maxnode/average`);

    return Promise.resolve(coins.map(c => ({ ...c, avgBlockTime, avgMNTime, avgMAXTime })));
  } catch(err) {
...
// Request the list of masternodes.
const getMAXs = query => fetch(`${ api }/maxnode`, query);
